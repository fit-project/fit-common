...
def get_vb_cable_virtual_audio_device():
    for dev in QMediaDevices().audioInputs():
        if any(x in dev.description() for x in ["Virtual Cable", "CABLE Output", "VB-Audio", "VB-Cable"]):
            return dev
    return None
...
def enable_audio_recording():
    return (is_installed_ffmpeg() and get_vb_cable_virtual_audio_device() and is_vb_cable_first_ouput_audio_device())
